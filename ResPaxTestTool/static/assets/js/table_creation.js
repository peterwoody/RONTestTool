            ...
            if (typeof json.tours === 'string') {
                ...
                if (json.tours.length > 0) {
                    ...
                    for (var i = 0; i in tour_codes; i++) {
                        var k = 0;
                        while (tour_codes[k]['strTourCode'] !== keys[i] || j.includes(k) === true) {
                            k++;
                        }
                        j.push(k);
        ...
        success: function (json) {
            ...
            if (!jQuery.isEmptyObject(json.tour_bases)) {
                ...
                for (var i = 0; i < json.tour_bases.length; i++) {
                    ...
                    while (tour_bases[k]['strBasisDesc'] !== keys[i] || j.includes(k) === true) {
                        k++;
                    }
                    j.push(k);
                    var tour_basis_name = tour_bases[k]['strBasisDesc'];
                    var tour_sub_basis_name = tour_bases[k]['strSubBasisDesc'];
                    ...
                    var basis_td_value = document.createTextNode("Basis: " + tour_basis_name + ' (' + tour_basis_id + ')');

                    var subbasis_td_value = document.createTextNode("Subbasis: " + tour_sub_basis_name + ' (' + tour_sub_basis_id + ')');
                    var string_p = document.createElement("p");
                    var string_value = document.createTextNode("(" +
                        host_id + "," +
                        tour_code + "," +
        ...
        success: function (json) {
            ...
            if (!jQuery.isEmptyObject(json.tour_times)) {
                ...
                for (var i = 0; i < json.tour_times.length; i++) {
                    var tour_times = json.tour_times;

                    var keys = get_sorted_keys(tour_times, "dteTourTime");

                    var k = 0;
                    while (tour_times[k]['dteTourTime'] !== keys[i] || j.includes(k) === true) {
                        k++;
                    }
                    j.push(k);
                    ...
                    var time_td_value = document.createTextNode("Time: " + tour_time + ' (' + tour_time_id + ')');
                    var string_p = document.createElement("p");
                    var string_value = document.createTextNode("(" +
                        host_id + "," +
                        tour_code + "," +
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            for (var i = 0; i < json.tour_pickups.length; i++) {
                ...
                var k = 0;
                while (tour_pickups[k]['strPickupValue'] !== keys[i] || j.includes(k) === true) {
                    k++;
                }
                j.push(k);
                ...
                var pickup_id_td_value = document.createTextNode("Pickup ID: " + tour_pickup_value + ' ' + '(' + tour_pickup_key + ')');
